...
Partial Public Class AnimalCorral
    ...
    Private Sub CargarCorrales()
        Try
            Dim query As String = "SELECT ID_Corral, Nombre_Corral FROM Corral ORDER BY Nombre_Corral"
            Dim corralesData As DataTable = DataAccess.ExecuteSelectQuery(query)
            
            ddlCorral.DataSource = corralesData
            ddlCorral.DataTextField = "Nombre_Corral"
            ddlCorral.DataValueField = "ID_Corral"
            ddlCorral.DataBind()
            ddlCorral.Items.Insert(0, New ListItem("Seleccione un corral", "0"))
        Catch ex As Exception
    ...
    Private Sub CargarMovimientos()
        Try
            ...
            Dim checkTableQuery As String = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'Animal_Corral'"
            Dim tableExists As Object = DataAccess.ExecuteScalar(checkTableQuery)
            
            If Convert.ToInt32(tableExists) = 0 Then
                MostrarAlerta("La tabla 'Animal_Corral' no existe en la base de datos. Por favor, créala primero.", "warning")
                Return
            End If
            
            ' Obtener movimientos con JOINs para mostrar nombres
            Dim query As String = "SELECT AC.ID_Animal, A.Nombre_Animal, AC.ID_Corral, C.Nombre_Corral, AC.Fecha_Entrada, AC.Fecha_Salida " & _
                                  "FROM Animal_Corral AC " & _
                                  "INNER JOIN Animal A ON AC.ID_Animal = A.ID_Animal " & _
                                  "INNER JOIN Corral C ON AC.ID_Corral = C.ID_Corral " & _
                                  "ORDER BY AC.Fecha_Entrada DESC, A.Nombre_Animal"
    ...
    Protected Sub btnEliminarOculto_Click(sender As Object, e As EventArgs)
        Try
            ...
            EliminarMovimiento(animalId, corralId, fechaEntrada)
        Catch ex As Exception
    ...
    Protected Sub btnGuardar_Click(sender As Object, e As EventArgs)
        Try
            If ValidarFormulario() Then
                ...
                Dim corralId As Integer = Convert.ToInt32(ddlCorral.SelectedValue)
                ...
                Dim editando As Boolean = Not String.IsNullOrEmpty(hfAnimalId.Value) AndAlso Not String.IsNullOrEmpty(hfCorralId.Value) AndAlso Not String.IsNullOrEmpty(hfFechaEntrada.Value)
                
                Dim result As Integer

                If Not editando Then
                    ' Insertar nuevo movimiento
                    If ExisteMovimiento(animalId, corralId, fechaEntrada) Then
                        MostrarAlerta("Este animal ya está registrado en este corral en la fecha de entrada seleccionada", "warning")
                        Return
                    End If

                    result = InsertarMovimiento(animalId, corralId, fechaEntrada, fechaSalida)
                ...
                Else
                    ...
                    result = ActualizarMovimiento(animalId, corralId, fechaEntrada, fechaSalida)
            ...
            Select Case e.CommandName
                Case "EditarMovimiento"
                    If args.Length = 3 Then
                        ...
                        Dim corralId As Integer = Convert.ToInt32(args(1))
                        Dim fechaEntrada As Date = Convert.ToDateTime(args(2))
                        EditarMovimiento(animalId, corralId, fechaEntrada)
                    End If
    ...
    Private Sub EditarMovimiento(animalId As Integer, corralId As Integer, fechaEntrada As Date)
        Try
            Dim query As String = "SELECT ID_Animal, ID_Corral, Fecha_Entrada, Fecha_Salida FROM Animal_Corral WHERE ID_Animal = @ID_Animal AND ID_Corral = @ID_Corral AND Fecha_Entrada = @Fecha_Entrada"
            ...
            If movimientoData.Rows.Count > 0 Then
                ...
                ' Cargar y seleccionar Corral (permitir cambiar)
                If ddlCorral.Items.Count <= 1 Then
                    CargarCorrales()
                End If
                ddlCorral.SelectedValue = row("ID_Corral").ToString()
                ...
                modalTitle.InnerText = "Editar Movimiento de Corral"
    ...
    Private Sub EliminarMovimiento(animalId As Integer, corralId As Integer, fechaEntrada As Date)
        Try
            ' Usar procedimiento almacenado para eliminar
            Dim query As String = "EXEC EliminarAnimalCorral @ID_Animal, @ID_Corral, @Fecha_Entrada"
            Dim parameters() As SqlParameter = {
                New SqlParameter("@ID_Animal", animalId),
                New SqlParameter("@ID_Corral", corralId),
                New SqlParameter("@Fecha_Entrada", fechaEntrada)
    ...
    Private Function InsertarMovimiento(animalId As Integer, corralId As Integer, fechaEntrada As Date, fechaSalida As Date?) As Integer
        Try
            ' Usar procedimiento almacenado para insertar
            Dim query As String = "EXEC InsertarAnimalCorral @ID_Animal, @ID_Corral, @Fecha_Entrada, @Fecha_Salida"
            Dim parameters() As SqlParameter = {
                New SqlParameter("@ID_Animal", animalId),
                New SqlParameter("@ID_Corral", corralId),
                New SqlParameter("@Fecha_Entrada", fechaEntrada),
    ...
    Private Function ActualizarMovimiento(animalId As Integer, corralId As Integer, fechaEntrada As Date, fechaSalida As Date?) As Integer
        Try
            ' Usar procedimiento almacenado para actualizar
            Dim query As String = "EXEC ActualizarAnimalCorral @ID_Animal, @ID_Corral, @Fecha_Entrada, @Fecha_Salida"
            Dim parameters() As SqlParameter = {
                New SqlParameter("@ID_Animal", animalId),
                New SqlParameter("@ID_Corral", corralId),
                New SqlParameter("@Fecha_Entrada", fechaEntrada),
    ...
    Private Function ExisteMovimiento(animalId As Integer, corralId As Integer, fechaEntrada As Date) As Boolean
        Try
            Dim query As String = "SELECT COUNT(*) FROM Animal_Corral WHERE ID_Animal = @ID_Animal AND ID_Corral = @ID_Corral AND Fecha_Entrada = @Fecha_Entrada"
            Dim parameters() As SqlParameter = {
                New SqlParameter("@ID_Animal", animalId),
                New SqlParameter("@ID_Corral", corralId),
                New SqlParameter("@Fecha_Entrada", fechaEntrada)
    ...
    Private Function ValidarFormulario() As Boolean
        ...
        If ddlCorral.SelectedValue = "0" Then
            MostrarAlerta("Debe seleccionar un corral", "warning")
            Return False
        End If
    ...
    Private Sub LimpiarFormulario()
        ...
        End If
        If ddlCorral.Items.Count > 0 Then
            ddlCorral.SelectedIndex = 0
        End If
        ...
        modalTitle.InnerText = "Nuevo Movimiento a Corral"
    End Sub
